...
public class SpeechToTextPlugin :
    ...
    private fun startListening(result: Result, languageTag: String, partialResults: Boolean,
                               listenModeIndex: Int, onDevice: Boolean, pauseFor: Int?, mixWithOthersParam: Boolean) {
        ...
        // Save the mixWithOthers preference and request audio focus accordingly
        mixWithOthers = mixWithOthersParam
        val focusGranted = requestAudioFocus(mixWithOthers)
        debugLog("Audio focus requested. granted=$focusGranted, mixWithOthers=$mixWithOthers")

        handler.post {
            run {
                // proceed to start listening regardless of focus result (some devices may not grant focus)
...
//            var recognizerIntent = Intent(RecognizerIntent.ACTION_GET_LANGUAGE_DETAILS)
    ...
    private fun notifyListening(isRecording: Boolean ) {
        ...
        if ( !isRecording ) {
            ...
            // release audio focus when we are done
            abandonAudioFocus()
    ...
    // ----------------- Audio focus helpers -----------------
    private fun requestAudioFocus(mixWithOthers: Boolean): Boolean {
        val ctx = pluginContext ?: return false
        val am = audioManager ?: ctx.getSystemService(Context.AUDIO_SERVICE) as AudioManager?
        if (am == null) {
            debugLog("AudioManager null, cannot request audio focus")
            return false
        }

        // create listener if not present
        if (audioFocusChangeListener == null) {
            audioFocusChangeListener = AudioManager.OnAudioFocusChangeListener { focusChange ->
                // handle focus changes if necessary (ducking, loss, gain)
                debugLog("Audio focus change: $focusChange")
                // We could notify the Dart side if needed
            }
        }

        // For API 26+, use AudioFocusRequest.Builder
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val focusGainType = if (mixWithOthers) {
            ...
            try {
                val afrBuilder = AudioFocusRequest.Builder(focusGainType)
                    .setAudioAttributes(audioAttributes)
                    .setAcceptsDelayedFocusGain(true)
                    .setOnAudioFocusChangeListener(audioFocusChangeListener!!)
                audioFocusRequest = afrBuilder.build()
                val res = am.requestAudioFocus(audioFocusRequest!!)
                debugLog("requestAudioFocus (API>=26) result: $res")
                return res == AudioManager.AUDIOFOCUS_REQUEST_GRANTED
            } catch (e: Exception) {
                debugLog("Exception requesting audio focus (API>=26): ${e.localizedMessage}")
        ...
        val focusGainTypeLegacy = if (mixWithOthers) {
            AudioManager.AUDIOFOCUS_GAIN_TRANSIENT_MAY_DUCK
        } else {
            AudioManager.AUDIOFOCUS_GAIN_TRANSIENT
        }
        val resLegacy = am.requestAudioFocus(
            audioFocusChangeListener,
            stream,
            focusGainTypeLegacy
        )
        debugLog("requestAudioFocus (legacy) result: $resLegacy")
        return resLegacy == AudioManager.AUDIOFOCUS_REQUEST_GRANTED
    }
    ...
    private fun abandonAudioFocus() {
        ...
        val am = audioManager ?: ctx.getSystemService(Context.AUDIO_SERVICE) as AudioManager?
        if (am == null) {
            debugLog("AudioManager null, cannot abandon audio focus")
            return
        }
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                if (audioFocusRequest != null) {
                    val res = am.abandonAudioFocusRequest(audioFocusRequest!!)
                    debugLog("abandonAudioFocusRequest result: $res")
                } else {
                    // Fallback: abandon without request
            ...
            debugLog("Exception abandoning audio focus: ${e.localizedMessage}")
            try {
                am.abandonAudioFocus(audioFocusChangeListener)
